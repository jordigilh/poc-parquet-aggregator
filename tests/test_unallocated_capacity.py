...
class TestUnallocatedCapacityAggregator:
    ...
    def sample_node_roles(self):
        ...
        """
        return pd.DataFrame({
            'node': ['master-0', 'master-1', 'infra-0', 'worker-0', 'worker-1', 'worker-2'],
            'resource_id': ['i-master0', 'i-master1', 'i-infra0', 'i-worker0', 'i-worker1', 'i-worker2'],
            'node_role': ['master', 'master', 'infra', 'worker', 'worker', 'worker']
        })
    ...
    def sample_daily_summary(self):
        ...
        return pd.DataFrame({
            ...
            'node': ['master-0', 'master-0', 'infra-0', 'worker-0', 'worker-1', 'worker-2'],
            'resource_id': ['i-master0', 'i-master0', 'i-infra0', 'i-worker0', 'i-worker1', 'i-worker2'],
            ...
            'pod_effective_usage_memory_gigabyte_hours': [1.0, 2.0, 2.0, 6.0, 10.0, 4.0],
            'node_capacity_cpu_core_hours': [24.0, 24.0, 24.0, 96.0, 96.0, 96.0],
            'node_capacity_memory_gigabyte_hours': [64.0, 64.0, 64.0, 256.0, 256.0, 256.0],
    ...
    def test_node_role_lookup_with_max_aggregation(self, config):
        ...
        node_roles_raw = pd.DataFrame({
            ...
            'node_role': ['worker', 'infra', 'master']  # node-0 has both worker and infra
        })
    ...
    def test_platform_unallocated_for_master_nodes(self, sample_daily_summary, sample_node_roles, config):
        ...
        result = aggregator.calculate_unallocated(
            daily_summary_df=sample_daily_summary,
            node_roles_df=sample_node_roles
        )
    ...
    def test_output_has_required_columns(self, sample_daily_summary, sample_node_roles, config):
        ...
        required_columns = [
            ...
            'data_source', 'source_uuid', 'cluster_id', 'cluster_alias',
            'report_period_id'
        ]
    ...
    def test_handles_node_without_role(self, sample_daily_summary, config):
        ...
        result = aggregator.calculate_unallocated(
            daily_summary_df=sample_daily_summary,
            node_roles_df=incomplete_roles
        )
    ...
    def test_handles_empty_daily_summary(self, sample_node_roles, config):
        ...
        result = aggregator.calculate_unallocated(
            daily_summary_df=empty_summary,
            node_roles_df=sample_node_roles
        )
    ...
    def test_negative_unallocated_becomes_zero(self, sample_node_roles, config):
        ...
        result = aggregator.calculate_unallocated(
            daily_summary_df=over_provisioned,
            node_roles_df=sample_node_roles
        )
