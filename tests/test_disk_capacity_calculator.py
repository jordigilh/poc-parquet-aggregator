...
def sample_ocp_storage():
    ...
    return pd.DataFrame({
        ...
        'csi_volume_handle': ['vol-0123456789abcdef', 'vol-0123456789abcdeg', 'vol-0123456789abcdeh'],
...
def sample_aws_ebs_data():
    ...
    return pd.DataFrame({
        ...
        'lineitem_productcode': [
            'AmazonEBS', 'AmazonEBS', 'AmazonEBS', 'AmazonEBS', 'AmazonEBS'
        ],
...
class TestDiskCapacityCalculator:
    ...
    def test_calculate_disk_capacities_formula(
        self, mock_config
    ):
    ...
    def test_calculate_disk_capacities_max_aggregation(
        self, mock_config
    ):
    ...
    def test_calculate_disk_capacities_no_matching_volumes(
        self, mock_config
    ):
    ...
    def test_same_volume_multiple_days(self, mock_config):
        ...
        ocp_storage = pd.DataFrame({
            'csi_volume_handle': ['vol-multiday']
        })
    ...
    def test_capacity_rounding_boundaries(self, mock_config):
        ...
        aws_data = pd.DataFrame({
            ...
            'lineitem_usagestartdate': pd.to_datetime(['2025-10-01', '2025-10-01', '2025-10-01']),
            'lineitem_unblendedcost': [1.7938, 1.8120, 1.8000],  # Results in 99.5, 100.5, 100.0
            'lineitem_unblendedrate': [0.0134, 0.0134, 0.0134]
        })
    ...
    def test_negative_costs_credits(self, mock_config):
        ...
        aws_data = pd.DataFrame({
            'lineitem_resourceid': ['vol-positive', 'vol-negative-credit', 'vol-mixed'],
            'lineitem_usagestartdate': pd.to_datetime(['2025-10-01', '2025-10-01', '2025-10-01']),
            'lineitem_unblendedcost': [10.0, -5.0, 0.5],  # Positive, negative, small positive
            'lineitem_unblendedrate': [0.0134, 0.0134, 0.0134]
        })
    ...
    def test_extreme_cost_values(self, mock_config):
        ...
        aws_data = pd.DataFrame({
            ...
            'lineitem_usagestartdate': pd.to_datetime(['2025-10-01', '2025-10-01', '2025-10-01']),
            'lineitem_unblendedcost': [1_000_000.0, 0.0001, 10.0],  # Enterprise, micro, normal
            'lineitem_unblendedrate': [0.0134, 0.0134, 0.0134]
        })
    ...
    def test_null_vs_zero_costs_rates(self, mock_config):
        ...
        ocp_storage = pd.DataFrame({
            'csi_volume_handle': ['vol-null-cost', 'vol-zero-cost', 'vol-null-rate', 'vol-zero-rate']
        })

        aws_data = pd.DataFrame({
            'lineitem_resourceid': ['vol-null-cost', 'vol-zero-cost', 'vol-null-rate', 'vol-zero-rate'],
    ...
    def test_hourly_rate_fluctuations(self, mock_config):
        ...
        ocp_storage = pd.DataFrame({
            'csi_volume_handle': ['vol-rate-change']
        })
    ...
    def test_month_boundary_calculations(self, mock_config):
        ...
        ocp_storage = pd.DataFrame({
            'csi_volume_handle': ['vol-boundary']
        })
    ...
    def test_duplicate_resource_ids_same_day(self, mock_config):
        ...
        ocp_storage = pd.DataFrame({
            'csi_volume_handle': ['vol-duplicate']
        })
        ...
        aws_data = pd.DataFrame({
            ...
            'lineitem_unblendedrate': [0.010, 0.012, 0.015, 0.008, 0.013]  # MAX = 0.015
        })
