...
def main():
    ...
    for file_type, (patterns, s3_dir) in file_types.items():
        ...
        for csv_file in csv_files:
            try:
                ...
                if len(df_temp) > 0:
                    ...
                    print(f"   ⚠️  {csv_file.name}: empty, skipping")
            except Exception as e:
                print(f"   ❌ {csv_file.name}: failed to read - {e}")

        if not all_dfs:
            print(f"   ⚠️  No valid data found for {file_type}")
            continue
        ...
        if len(date_groups) == 0:
            print(f"   ⚠️  No valid days found, skipping")
            continue
