...
# Continuous memory + CPU sampling at ~100ms interval (industry standard)
while proc.poll() is None:
    try:
        p = psutil.Process(proc.pid)
        rss = p.memory_info().rss / (1024**2)
...
" 2>/dev/null || echo "0")
    ...
    echo -e "${GREEN}    Run $run_num: ${agg_time}s, ${peak_memory}MB, CPU avg=${avg_cpu}% max=${max_cpu}%, $output_rows rows, $throughput rows/s${NC}" >&2

    # Return values via file (now includes CPU)
...
- Mode: OCP-only (in-memory, **single-threaded**)
